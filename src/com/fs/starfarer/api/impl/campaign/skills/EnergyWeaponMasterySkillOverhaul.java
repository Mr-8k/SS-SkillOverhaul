...
public class EnergyWeaponMasterySkillOverhaul {
	...
	public static class Level1 extends BaseSkillEffectDescription implements AfterShipCreationSkillEffect {
		...
		public void createCustomDescription(MutableCharacterStatsAPI stats, SkillSpecAPI skill, 
			...
			if (ENERGY_DAMAGE_MIN_FLUX_LEVEL > 0) {
				info.addPara("Energy weapons deal up to %s damage as the firing ship's flux level increases above %s. Always grants at least %s damage.",
						0f, hc, hc,
						"+" + (int) ENERGY_DAMAGE_PERCENT + "%",
						(int) Math.round(ENERGY_DAMAGE_MIN_FLUX_LEVEL * 100f) + "%"
				);
			} else {
				info.addPara("Energy weapons deal up to %s damage, based on the firing ship's flux level. Always grants at least %s damage.",
						0f, hc, hc,
						"+" + (int) ENERGY_DAMAGE_PERCENT + "%",
						"+" + (int) ENERGY_DAMAGE_PERCENT_MIN + "%"
				);
			}
	...
	public static class EWMDamageDealtMod implements DamageDealtModifier, AdvanceableListener {
		...
		public void advance(float amount) {
			if (Global.getCurrentState() == GameState.COMBAT &&
				...
				int damageBonus = (int) Math.round((max(ship.getFluxBasedEnergyWeaponDamageMultiplier() * 100f, ENERGY_DAMAGE_PERCENT_MIN + 100) - 100f));
				if (damageBonus > 5) {
		...
		public String modifyDamageDealt(Object param,
			...
			float mag = max(ship.getFluxBasedEnergyWeaponDamageMultiplier() - 1f, ENERGY_DAMAGE_PERCENT_MIN * .01f);
