...
public class WolfpackTactics {
	...
	public static class Level1A implements ShipSkillEffect {
		public void apply(MutableShipStatsAPI stats, HullSize hullSize, String id, float level) {
			...
			if (isDestroyerAndOfficer(stats)) {
				stats.getDamageToDestroyers().modifyPercent(id, DAMAGE_TO_LARGER_BONUS/2);
				stats.getDamageToCruisers().modifyPercent(id, DAMAGE_TO_LARGER_BONUS/2);
				stats.getDamageToCapital().modifyPercent(id, DAMAGE_TO_LARGER_BONUS/2);

				stats.getPeakCRDuration().modifyFlat(id, PEAK_TIME_BONUS/2);
				stats.getDynamic().getMod(Stats.INDIVIDUAL_SHIP_RECOVERY_MOD).modifyFlat(id, 1000f);
			}
		}
		...
		public String getEffectDescription(float level) {
			...
			return "+" + (int)Math.round(DAMAGE_TO_LARGER_BONUS) + "% damage to ships larger than frigates (halved bonus for destroyers)\n" +
				   "+" + (int)(PEAK_TIME_BONUS) + " seconds peak operating time (halved bonus for destroyers)\n" +
	...
	public static class Level1B extends SkillValueInformation implements ShipSkillEffect {
		public void apply(MutableShipStatsAPI stats, HullSize hullSize, String id, float level) {
	...
	public static class Level1C extends SkillValueInformation implements AfterShipCreationSkillEffect {
